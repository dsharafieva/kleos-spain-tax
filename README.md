...
| Currency | **displayed in USD**, thresholds held in EUR (see NOTES.md) |
| Rules as of | 2026 |
| Build | 99,610 bytes, sha256 `ca8ee0b549596fb1…` |
...
var USD_PER_EUR = 1.1535;     // ECB reference rate, 7 August 2026
```

Every threshold stays in euros because that is how Spanish law sets them. Only `money()` and
`money0()` convert, so changing the exchange rate is a one-line edit and the bracket logic never
touches a converted figure. The input field is the single place the user speaks dollars; the
handler divides by the rate before storing. Prose figures are converted and **rounded** —
statutory constants get "about" or "roughly" rather than false precision to the cent.
...
- [ ] VAT was removed series-wide on 11 Aug 2026. The third tax panel and third stat
      tile now cover corporate tax and the payout, which also gives the SL calculator tab
      representation in that section. One thing was lost with it: an EU buyer is no longer
      told they self-account for the tax under the reverse charge, which is a genuine
      obligation on the payer. Add a single FAQ line if that matters.
